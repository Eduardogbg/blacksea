...
library RedBlackTreeLib {
    ...
    function values(
        ...
        assembly {
            function visit(current_) {
                if iszero(current_) {
                    leave
                } // If the current node is null, leave.
                ...
                if iszero(value_) {
                    value_ := sload(or(current_, _BIT_FULL_VALUE_SLOT))
                }
    ...
    function value(bytes32 ptr) internal view returns (uint256 result) {
        ...
        assembly {
            ...
            if iszero(result) {
                result := sload(or(ptr, _BIT_FULL_VALUE_SLOT))
            }
        }
    }
    ...
    function _unpack(
        bytes32 ptr
    ) private pure returns (uint256 nodes, uint256 key) {
        /// @solidity memory-safe-assembly
        assembly {
            ...
            for {
                ...
                if iszero(result) {
                    ...
                    for {
                        ...
                        if iszero(
                            eq(target, and(shr(R, packed), _BITMASK_KEY))
                        ) {
                            break
                        }
                        target := result
    ...
    function _update(
        ...
        assembly {
            ...
            function setKey(packed_, bitpos_, key_) -> result_ {
                result_ := or(
                    and(not(shl(bitpos_, _BITMASK_KEY)), packed_),
                    shl(bitpos_, key_)
                )
            }
            ...
            function rotate(nodes_, key_, L, R) {
                ...
                sstore(
                    or(nodes_, key_),
                    setKey(packed_, _BITPOS_PARENT, cursor_)
                )
            ...
            function insert(nodes_, cursor_, key_, x_) -> err_ {
...

                {
                    ...
                    for {
                        ...
                        if iszero(lt(x_, cValue_)) {
                            sstore(
                                s_,
                                setKey(cPacked_, _BITPOS_RIGHT, totalNodes_)
                            )
                            break
                        }
                ...
                for {
                ...
                } iszero(eq(key_, mload(0x00))) {

                } {
                    let packed_ := sload(or(nodes_, key_))
                    let parent_ := getKey(packed_, _BITPOS_PARENT)
                    let parentPacked_ := sload(or(nodes_, parent_))
                    if iszero(and(BR, parentPacked_)) {
                        break
                    }
                    ...
                    let R := mul(
                        eq(parent_, getKey(grandParentPacked_, 0)),
                        _BITPOS_RIGHT
                    )
            ...
            function removeFixup(nodes_, key_) {
                ...
                for {
                    ...
                    let R := mul(
                        eq(key_, getKey(parentPacked_, 0)),
                        _BITPOS_RIGHT
                    )
                    let L := xor(R, _BITPOS_RIGHT)
                    ...
                    if iszero(
                        and(BR, or(cursorLeftPacked_, cursorRightPacked_))
                    ) {
                    ...
                    sstore(
                        ...
                        xor(
                            cursorPacked_,
                            and(BR, xor(cursorPacked_, parentPacked_))
                        )
                    )
                    sstore(or(nodes_, parent_), and(parentPacked_, not(BR)))
                    sstore(
                        or(nodes_, cursorRight_),
                        and(cursorRightPacked_, not(BR))
                    )
                    rotate(nodes_, parent_, L, R)
                    break
                }
            ...
            function remove(nodes_, key_) -> err_ {
                ...
                let cursor_ := key_
                {
                    ...
                    if mul(left_, right_) {
                        for {
                            ...
                            let cursorLeft_ := getKey(
                                sload(or(nodes_, cursor_)),
                                _BITPOS_LEFT
                            )
                            if iszero(cursorLeft_) {
                                break
                            }
                            cursor_ := cursorLeft_
                        }
                ...
                probe_ := getKey(
                    cursorPacked_,
                    mul(iszero(probe_), _BITPOS_RIGHT)
                )
                ...
                sstore(
                    probeSlot_,
                    setKey(sload(probeSlot_), _BITPOS_PARENT, yParent_)
                )
                ...
                if iszero(eq(cursor_, key_)) {
                    ...
                    sstore(
                        leftSlot_,
                        setKey(sload(leftSlot_), _BITPOS_PARENT, cursor_)
                    )

                    let rightSlot_ := or(nodes_, getKey(packed_, _BITPOS_RIGHT))
                    sstore(
                        rightSlot_,
                        setKey(sload(rightSlot_), _BITPOS_PARENT, cursor_)
                    )
                    ...
                    sstore(
                        ...
                        xor(
                            cursorPacked_,
                            and(
                                xor(packed_, cursorPacked_),
                                sub(shl(_BITPOS_PACKED_VALUE, 1), 1)
                            )
                        )
                    )
                ...
                if iszero(lastValue_) {
                    lastValue_ := sload(
                        or(_BIT_FULL_VALUE_SLOT, or(nodes_, last_))
                    )
                    lastFullValue_ := lastValue_
                }

                let cursorValue_ := shr(
                ...
                if iszero(cursorValue_) {
                    cursorValue_ := sload(
                        or(_BIT_FULL_VALUE_SLOT, or(nodes_, cursor_))
                    )
                ...
                if iszero(eq(lastValue_, cursorValue_)) {
                    ...
                    for {
                        let lastParent_ := getKey(lastPacked_, _BITPOS_PARENT)
                    } 1 {
                ...
                if lastFullValue_ {
                    sstore(or(_BIT_FULL_VALUE_SLOT, or(nodes_, last_)), 0)
                }
    ...
    function _find(
        Tree storage tree,
        uint256 x
    ) private view returns (uint256 nodes, uint256 cursor, uint256 key) {
        ...
        assembly {
            ...
            for {
                ...
                if iszero(nodeValue) {
                    nodeValue := sload(
                        or(or(nodes, probe), _BIT_FULL_VALUE_SLOT)
                    )
                }
                if eq(nodeValue, x) {
                    key := cursor
                ...
                probe := and(
                    shr(mload(gt(x, nodeValue)), nodePacked),
                    _BITMASK_KEY
                )
            }
        }
